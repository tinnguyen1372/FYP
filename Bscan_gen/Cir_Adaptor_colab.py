...
class Cir_Adaptor():
    ...
    def run_cavity(self,iteration):
            ...
            self.rcavity = self.data['R_center_Bcavity']
            self.cx, self.cy, self.cz = (self.data['CenTrunk_X'],self.data['CenTrunk_Y'],0)
            
            self.cavity_height = self.data['h_Bcavity'] if 'h_Bcavity' in self.data.keys() else 0
            self.theta_cavity = self.data['theta_Bcavity']
            self.distance = self.data['r_Bcavity']
            self.init_cavity()
            ...
            cu.preprocess(self.generated_healthy_base + str(iteration) + ".png", res)
...
# print(f"#cavity: {str('{cav_x}')} {str('{cav_y}')} {str('{cav_z}')} {str('{cav_x}')} {str('{cav_y}')} 0.002 {self.rcavity} cavity")
#end_python:
...
#geometry_objects_read: {trunk_base[0]:.3f} {trunk_base[1]:.3f} {trunk_base[2]:.3f} {self.prefix}healthy.h5 {self.prefix}materials.txt
        '''
            ...
            rxnumber = 1
            rxcomponent = 'Ez'
            plt = mpl_plot("merged_output_data", merged_data_healthy, dt, rxnumber,rxcomponent)
    ...
    def run_healthy(self,iteration):
            ...
            # Convert the generated image to h5
            cu.preprocess(self.generated_healthy_base + str(iteration) + ".png", res,prefix= self.prefix)
            # cu.preprocess(self.generated_cavity_base + str(iteration) + ".png", res)
            # Write to materials.txt
            with open('{}materials.txt'.format(self.prefix), "w") as file:
                # file.write('#material: {} 0 1 0 trunk\n'.format(eps_trunk))
...
#geometry_objects_read: {trunk_base[0]:.3f} {trunk_base[1]:.3f} {trunk_base[2]:.3f} {self.prefix}healthy.h5 {self.prefix}materials.txt
    ...
    def init_cavity(self):
        self.input = './treetrunk_{}_{}.in'.format(np.round(self.distance,2),self.rcavity)
        self.filename = './cavcoord/cavcoord_{}_{}.txt'.format(np.round(self.distance,2),self.rcavity)
    ...
    def run(self):
        for iteration in range(self.start, self.start + self.count):
            ...
            while os.path.isfile(destination_file_healthy):
                destination_file_healthy = cu.increment_file_index(
