...
class Cir_Adaptor():
    ...
    def run_healthy(self,iteration):
        ...
        for i in range(self.bscan_num):
            cu.preprocess_multilayers(self.generated_healthy_base + str(iteration) + ".png", res,self.prefix, iteration, angle = i*int(360/self.bscan_num))
            config = f'''
...
#geometry_objects_read: {trunk_base[0]:.3f} {trunk_base[1]:.3f} {trunk_base[2]:.3f} {self.prefix}healthy.h5 {self.prefix}materials.txt
    ...
    def run_cavity(self,iteration):   
        ...
        for i in range(self.bscan_num):
            cu.preprocess_multilayers(self.generated_cavity_base + str(iteration) + ".png", res,self.prefix, iteration, angle = i*int(360/self.bscan_num))
