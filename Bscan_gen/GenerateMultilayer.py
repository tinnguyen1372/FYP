...
def save_image( filename, x_trunk_L1, y_trunk_L1, x_trunk_L2, y_trunk_L2, x_trunk_L3, y_trunk_L3,
                x_trunk_L4, y_trunk_L4,
                # x_trunk_L5, y_trunk_L5,
    ...
    plt.fill(x_trunk_L4, y_trunk_L4, color=layer4, linestyle='none')
...
def generate_and_save_data(trunk_count, filename):
    ...
    x_trunk_L4 = np.zeros((N_trunk, N_angle))
    y_trunk_L4 = np.zeros((N_trunk, N_angle))
    r_trunk_L4 = np.zeros((N_trunk, N_angle))
    ...
    eps_bark = generate_random_array(N_trunk, 7.5, 2, 0.1, 5, 7.5)
    eps_cambium = generate_random_array(N_trunk, 12.5, 1, 0.1, 10, 15)
    eps_sapwood = generate_random_array(N_trunk, 12.5, 1, 0.1, 10, 15)
    eps_heartwood = generate_random_array(N_trunk, 7.5, 1, 0.1, 5, 7.5)
    eps_cavity = 1
    up_HW =0.4
    lo_HW=0.2
    ...
    for i in range(N_trunk):
        ...
        ratio_L4 = np.random.rand()*(up_HW-lo_HW) + up_HW
        ...
        Rmin_trunk_L4 = np.min(r_trunk_L4[i])
        Rmax_Bcavity = 0.8 * Rmin_trunk_L4
        Rmin_Bcavity = 0.05
    ...
    np.savez('SL_trunk.npz', 
            ...
            # eps_trunk = eps_trunk),
            eps_bark = eps_bark,
            eps_cambium = eps_cambium,
            eps_sapwood =  eps_sapwood,
            eps_heartwood = eps_heartwood)
    ...
    return {
        ...
        'x_trunk_L4': x_trunk_L4,
        'y_trunk_L4': y_trunk_L4,

        'x_Bcavity': x_Bcavity,
        'y_Bcavity': y_Bcavity,
        'range_trunk': range_trunk,
        'hole_factor': hole_factor,
        'r_trunk_L1': r_trunk_L1,
        'r_trunk_L2': r_trunk_L2,
        'r_trunk_L3': r_trunk_L3,

        'r_trunk_L4': r_trunk_L4,

        'R_center_Bcavity': R_center_Bcavity,
        'theta_Bcavity': theta_Bcavity,
        'r_Bcavity': r_Bcavity,
        'CenBcavity_X': CenBcavity_X,
        'CenBcavity_Y': CenBcavity_Y,
        'CenTrunk_X': CenTrunk_X,
        'CenTrunk_Y': CenTrunk_Y,
        'radius' : radius,
        # 'eps_trunk' : eps_trunk,
        'eps_bark' : eps_bark,
        'eps_cambium' : eps_cambium,
