...
# ax[0].imshow(np.transpose(dset[:,40,:], axes=(1, 0)), cmap='viridis') #antenna
ax[0].imshow(np.transpose(dset[:,40,:], axes=(1, 0)), cmap='viridis') #antenna
ax[0].invert_yaxis()
ax[0].set_title('Domain View')

# # Plot dset2 on the right subplot
ax[1].imshow(np.transpose(dset[:,:,349], axes=(1, 0)), cmap='viridis')
ax[1].invert_yaxis()
ax[1].set_title('Trunk View')

ax[2].imshow(np.transpose(dset[:,:,349], axes=(1, 0)), cmap='viridis')
ax[2].invert_yaxis()
