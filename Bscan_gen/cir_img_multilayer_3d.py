...
class Cir_Adaptor():
    ...
    def run_healthy(self,iteration):
        ...
        eps_bark = self.data['eps_bark'][iteration]
        eps_cambium = self.data['eps_cambium'][iteration]
        eps_sapwood = self.data['eps_sapwood'][iteration]
        eps_heartwood = self.data['eps_heartwood'][iteration]
        ...
        with open('{}materials.txt'.format(self.prefix), "w") as file:
            ...
            file.write('#material: {} 0 1 0 cambium\n'.format(eps_cambium))
...
antenna_like_GSSI_1500({src_position[0]:.3f}, {src_position[1]:.3f}, {src_position[2]:.3f}, resolution=0.002)
#end_python:
...
#geometry_objects_read: {trunk_base[0]:.3f} {trunk_base[1]:.3f} {trunk_base[2]:.3f} {self.prefix}healthy.h5 {self.prefix}materials.txt
    ...
    '''
        ...
        with h5py.File('cir_src_only_3d.out', 'r') as f1:
            data_source = f1['rxs']['rx1']['Ey'][()]

        src = data_source
        src = src[:, np.newaxis]
        Ey0 = np.repeat(src, self.bscan_num, axis=1)

        self.merged_data_healthy = np.subtract(data1, Ey0)

        # Create a new output file and write the merged data
        with h5py.File(output_filename, 'w') as f_out:
            f_out.attrs['dt'] = dt  # Set the time step attribute
            f_out.create_dataset('rxs/rx1/Ey', data=self.merged_data_healthy)
    ...
    def run_cavity(self,iteration):   
        ...
        for i in range(self.bscan_num):
            cu.preprocess_multilayers_3d(self.generated_cavity_base + str(iteration) + ".png", res,self.prefix, iteration, angle = i*int(360/self.bscan_num),z_layers=int(height))
...
#geometry_objects_read: {trunk_base[0]:.3f} {trunk_base[1]:.3f} {trunk_base[2]:.3f} {self.prefix}cavity.h5 {self.prefix}materials.txt
    ...
    '''
        ...
        src = src[:, np.newaxis]
        Ey0 = np.repeat(src, self.bscan_num, axis=1)
        self.merged_data_cavity = np.subtract(data1, Ey0)

        # Create a new output file and write the merged data
        with h5py.File(output_filename, 'w') as f_out:
            f_out.attrs['dt'] = dt  # Set the time step attribute
            f_out.create_dataset('rxs/rx1/Ey', data=self.merged_data_cavity)
