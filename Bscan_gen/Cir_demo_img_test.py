...
class Cir_Adaptor():
    ...
    def run_healthy(self,iteration):
        ...
        for i in range(self.bscan_num):
            cu.preprocess(self.generated_healthy_base + str(iteration) + ".png", res,self.prefix, iteration, angle = i*int(360/self.bscan_num))
            config = f'''
...
#geometry_objects_read: {trunk_base[0]:.3f} {trunk_base[1]:.3f} {trunk_base[2]:.3f} {self.prefix}healthy.h5 {self.prefix}materials.txt
    ...
    def run_cavity(self,iteration):   
        ...
        x_gap = 0.02
        y_gap = 0.02
        src_to_pml = 0.01 #0.02
        src_to_trunk = 0.03 #0.1
        sharp_domain = diameter , diameter + src_to_trunk
        z = 0
        domain = [
            sharp_domain[0] + pml * 2 + x_gap ,
        ...
        for i in range(self.bscan_num):
            cu.preprocess(self.generated_cavity_base + str(iteration) + ".png", res,self.prefix, iteration, angle = i*int(360/self.bscan_num))
